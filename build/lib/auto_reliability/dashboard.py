...
def _render_selector(st: Any, catalog: Any) -> tuple[str | None, str | None, int | None, bool]:
    ...
    with st.container(border=True):
        ...
        with action_left:
            evaluate = st.button("Estimar propensión a recalls", type="primary", use_container_width=True)
        with action_right:
            st.button("Reiniciar", use_container_width=True, on_click=_reset_dashboard, args=(st,))
...
def _render_downloads(st: Any, service: Any | None, result: Mapping[str, Any]) -> None:
    ...
    with middle:
        if pdf_state_key in st.session_state:
            ...
            if st.button("Preparar informe PDF", key=f"prepare_{pdf_state_key}", use_container_width=True):
...
def _render_result(st: Any, service: Any | None, result: Mapping[str, Any]) -> None:
    ...
    with action_left:
        if st.button("Añadir a comparación", key=f"compare_{_result_key(result)}", use_container_width=True):
    ...
    with factors_column:
        ...
        st.dataframe(_factor_table(result), use_container_width=True, hide_index=True)
...
def _render_real_recalls_tab(st: Any, service: Any | None) -> None:
    ...
    submitted = st.button("Consultar fuente oficial NHTSA", type="primary", use_container_width=True)
    if submitted:
