...
* **Variable objetivo (`indice_fiabilidad_100`):** Variable numérica continua (escala de 0 a 100, donde 100 es máxima fiabilidad).
* **Justificación y cálculo de la variable objetivo:** Entendiendo que los *recalls* son un proxy legal y no una métrica de fiabilidad mecánica directa, no se puede usar un conteo bruto. La métrica se calculará en tres fases para asegurar su comparabilidad:
    ...
    2. **Mitigación del sesgo de antigüedad (Anualización):** La suma de los pesos de gravedad de un modelo se dividirá entre los años transcurridos desde su fabricación (Año actual - Año fabricación + 1). Esto evita penalizar a los coches más antiguos que naturalmente acumulan más revisiones.
    3. **Mitigación del sesgo de volumen y escalado:** Dado que los coches más vendidos (ej. Ford F-150) estadísticamente sufren más recalls brutos, la tasa anualizada se estandarizará utilizando el *Z-score* (media y desviación estándar) **dentro de su propio segmento de mercado** (ej. comparar Pickups contra Pickups, Sedanes contra Sedanes). El resultado se escalará a un rango de 0 a 100. Esto hace que un "90 de fiabilidad en un SUV" sea matemáticamente comparable con un "90 en un Deportivo".
...
* **Auditoría Accionable:** Para evitar falsos positivos al unir los datasets, el *pipeline* aplicará las siguientes reglas y umbrales concretos generados en `audit_fuzzy_matches.csv`:
...
| `score_recalls_bruto`| Suma ponderada de gravedad de recalls | float | NHTSA | Sí | Sin normalizar |
| `indice_fiabilidad_100`| Variable objetivo final | float | Derivado | Sí | Z-score intra-segmento, escalado 0-100 |
...
* **Discrepancia semántica (Nomenclaturas):** Diferencias en la forma de escribir los modelos (tratado vía Fuzzy Matching auditado).
...
* **Feature Engineering:** Cálculo de variables relativas y ejecución de la normalización matemática del `indice_fiabilidad_100`.

---

## 9. Riesgos del modelo de datos
* **La parte más clara:** La disponibilidad del dataset de Kaggle y la estructuración de la ingesta desde la API.
* **La mayor incertidumbre:** El porcentaje de éxito real del algoritmo de *Fuzzy Matching* tras la revisión manual de los casos dudosos.
* **Mitigación y Plan de Contingencia:** Si tras evaluar el archivo `audit_fuzzy_matches.csv`, la tasa de pérdida de modelos (registros huérfanos) supera el 40%, se activará el plan alternativo. Este consiste en prescindir de los datos de averías de la API y utilizar exclusivamente la base de datos de Kaggle para predecir la **depreciación del vehículo** (cruzando el MSRP original de Kaggle con un *scraper* de precios actuales de segunda mano en portales de motor para calcular la pérdida de valor según sus características técnicas).
